...
var app_uri = 'http://nodejsworkspace.nesslersreagent.c9.io/';
...
function reaper_status(req, res){
  ...
  coll_reaper_data.count(function (err, num_of_docs) {
    ...
    coll_reaper_data.find( {}, {'date': true, 'D.T': true, 'D.S': true, 'D.N': true} ).toArray(function (err, data) {
      ...
      latest_log_date = data[data.length-1].date.toLocaleDateString();
      //console.log('Latest entry: ');
      //console.log( data[i_data].D[i_data_D].N.toString() + ' ' + data[i_data].D[i_data_D].S.toString() + ' ' + data[i_data].D[i_data_D].T.toISOString()  );
      res.render('reaper-nr', {num_of_logs: num_of_logs, latest_log_date: latest_log_date});  
    });
  });
}
...
function log_file_download(req, res){
  ...
  coll_reaper_data.find( {}, {'date': true, 'D': true} ).toArray(function (err, data) {
    i_data = data.length - 1;
    i_data_D = data[i_data].D.length - 1;
    latest_log_date = data[i_data].date;
